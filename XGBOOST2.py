...
train_data = pd.read_csv(r'F:\火狐文件\train.csv')
test_data = pd.read_csv(r'F:\火狐文件\test\test.csv')
...
XGB = XGBClassifier(nthread=4,  # 含义：nthread=-1时，使用全部CPU进行并行运算（默认）, nthread=1时，使用1个CPU进行运算。
                    ...
                    n_estimators=60,  # 含义：总共迭代的次数，即决策树的个数  50
                    max_depth=5,  # 含义：树的深度，默认值为6，典型值3-10。调参：值越大，越容易过拟合；值越小，越容易欠拟合
                    gamma=0,  # 含义：惩罚项系数，指定节点分裂所需的最小损失函数下降值。
                    subsample=0.95,  # 含义：训练每棵树时，使用的数据占全部训练集的比例。默认值为1，典型值为0.5-1。调参：防止过拟合
                    colsample_bytree=0.5)  # 训练每棵树时，使用的特征占全部特征的比例。默认值为1，典型值为0.5-1。调参：防止过拟合

score = cross_val_score(XGB, train_data, train_data_target, cv=10).mean()
...
sub.to_csv(r'F:\火狐文件\submission2.csv')